...
CONFIDENCE_THRESHOLD = float(os.getenv('CONFIDENCE_THRESHOLD', '0.85'))  # 置信度阈值
ADDITIONAL_VALIDATION = os.getenv('ADDITIONAL_VALIDATION', 'True').lower() in ('true', '1', 'yes')  # 启用额外验证机制
MIN_SCORE_GAP = float(os.getenv('MIN_SCORE_GAP', '0.1'))  # 最佳匹配与次佳匹配之间的最小分数差
DEBUG_MODE = os.getenv('DEBUG_MODE', 'False').lower() in ('true', '1', 'yes')  # 调试模式
RENAME_FILES = os.getenv('RENAME_FILES', 'True').lower() in ('true', '1', 'yes')  # 是否自动重命名文件
FILE_NAME_FORMAT = os.getenv('FILE_NAME_FORMAT', '{artist} - {title}')  # 文件名格式
SHOW_MULTIPLE_MATCHES = os.getenv('SHOW_MULTIPLE_MATCHES', 'True').lower() in ('true', '1', 'yes')  # 是否显示多个匹配候选
REQUIRE_CONFIRMATION = os.getenv('REQUIRE_CONFIRMATION', 'False').lower() in ('true', '1', 'yes')  # 是否要求人工确认
SKIP_EXISTING_TAGS = os.getenv('SKIP_EXISTING_TAGS', 'False').lower() in ('true', '1', 'yes')  # 是否跳过已有标签的文件
SHOW_TAG_COMPARISON = os.getenv('SHOW_TAG_COMPARISON', 'True').lower() in ('true', '1', 'yes')  # 是否显示新旧标签对比
LOW_CONFIDENCE_THRESHOLD = float(os.getenv('LOW_CONFIDENCE_THRESHOLD', '0.7'))  # 低置信度阈值
